...
function animateSwiper() {
	...
	var moreFlg = true;
	var introduceClick = function introduceClick() {
		swiper.disableTouchControl();
		$('#popupId').css({
			display: 'block'
		});
				...
				if (d.aaData.saoma) {
					...
					if (d.aaData.saoma.length > 0) {
						...
						for (var a = 0; a < arr.length; a++) {
							...
							var str = '<div>' + '<p><span>扫码时间：</span>' + arr[a].clickDate + '</p>' + '<p><span>地址：</span>' + name + '</p>' + '</div>';
							qrStr += str;
						}
					} else {
						$('#qrNum').html('共被查询1次');
...
function swiperList(arr) {
	...
	for (var m = 0; m < arr.length; m++) {
		strSwiper = '<div class="swiper-slide">' + '<img src="' + IMG_URL + arr[m] + '" alt="" />' + '<p>荣誉证照</p>' + '</div>';
...
function renderXsjlList(data) {
	...
	for (var m = 0; m < data.length; m++) {
		...
		if (m >= 1) {
			for (var n = 1; n < 2; n++) {
				htmlStr_left = '<li>' + '<span class="yellow_spot ani" swiper-animate-effect="bounceInDown" swiper-animate-duration="1s" swiper-animate-delay=' + animateC + 's></span>' + '<div class="text ani" swiper-animate-effect="bounceInLeft" swiper-animate-duration="1s" swiper-animate-delay=' + animateT + 's>' + '<p>批次号</p>' + '<p class="span">' + data[m][arr[m]].batchId + '</p>' + '<p>追溯码</p>' + '<p class="span">' + data[m][arr[m]].transId + '</p>' + '</div>' + '</li>';
				animateC = animateC + 0.8;
				animateT = animateT + 0.8;
			}
		}
		htmlStr_right = '<li>' + '<span class="yellow_spot ani" swiper-animate-effect="bounceInDown" swiper-animate-duration="1s" swiper-animate-delay=' + animateC + 's></span>' + '<div class="text ani" swiper-animate-effect="bounceInRight" swiper-animate-duration="1s" swiper-animate-delay=' + animateT + 's>' + '<p>' + data[m][arr[m]].salermarketName + '</p>' + '<p>' + dateText + data[m][arr[m]].indate.split(' ')[0] + '</p>' + '</div>' + '</li>';
...
function pinglun(arr) {
	...
	for (var m = 0; m < arr.length; m++) {
		...
		myEvaluateStr += '<div class="evaluateLook_list">' + '<div class="lookLeft">' + '<p class="lookTime">' + arr[m].createDate + '</p>' + '<div class="lookStar">' + '<div class="wrapper">' + starStr + '</div>' + '</div>' + '</div>' + '<div class="lookContent">' + '<p class="lookContentP" >' + arr[m].content + '</p>' + '</div>' + '<div class="mb_div_1_' + m + ' mb_div_1"></div>' + '</div>';
	}
